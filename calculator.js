...
function renderZoneStep(body, lang) {
  body.innerHTML = getStepHeader(1,'calcStep1','calcStep1sub') +
    CALC_ZONES.map(z => `
      <div class="zone-item ${selectedZone===z.key?'sel':''}" onclick="selectZone('${z.key}')">
        <div>
          <div class="zone-name">${lang==='uz'?z.uz:z.ru}</div>
          <div class="zone-sub">R стены: ${z.R_wall} • R кровли: ${z.R_roof} • R пол: ${z.R_floor} м²·К/Вт</div>
        </div>
        ${selectedZone===z.key?'<span style="font-size:18px">✅</span>':''}
      </div>`).join('');
}
