...
function doCalculate(){
  ...
  // Pack area depends on thickness: 50mm = packSlabs, 100mm = packSlabs/2
  const packArea = finalThick <= 50 ? prod.packArea : Math.round(1.2*0.6*Math.max(1,Math.floor(prod.packSlabs/2))*100)/100;
  const packs=Math.ceil(area*1.10/packArea);
  const vol=area*finalThick/1000;
  const weight=vol*prod.density;
  const price=Math.round(prod.pricePerM2*(finalThick/50)); // linear: 50mm=base price
...
function renderResult(body){
  ...
  const packAreaUsed = r.finalThick<=50?r.prod.packArea:Math.round(1.2*0.6*Math.max(1,Math.floor(r.prod.packSlabs/2))*100)/100;
  const f2='Packs = ⌈'+r.area.toFixed(1)+' × 1.1 / '+packAreaUsed+'⌉ = '+r.packs;
