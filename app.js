...
function buildProductImgHtml(prod) {
  if (prod.image) {
    return `<img class="prod-img" src="${prod.image}" alt="${prod.name}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
      ...
      <div class="prod-body">
        ...
        <div class="prod-actions">
          <div class="qty-ctrl">
            ...
            <input class="qty-val" id="qty-${prod.id}" type="number" min="1" value="1"
              oninput="setQty(${prod.id}, this.value)"
              onfocus="this.select()"
          ...
          <button class="detail-btn" onclick="addToCartFromCard(${prod.id})">${t('addCart')}</button>
        </div>
      </div>
    </div>`;
}
...
function changeQty(id, delta) {
  ...
  qtyState[id] = Math.max(1, current + delta);
  if (el) el.value = qtyState[id];
}
