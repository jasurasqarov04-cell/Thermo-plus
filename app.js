    ...
    <div class="overlay" id="detail-overlay" onclick="if(event.target===this)closeDetail()">
      <div class="sheet">
        ...
        <div class="sheet-body">
          ...
          <p style="font-size:12px;color:var(--text-2);line-height:1.6;margin-bottom:14px;">${desc}</p>
          <div style="background:var(--bg);border-radius:10px;padding:0 12px;margin-bottom:14px;">${specRows}</div>
          <div style="margin-bottom:14px;">
            <div style="font-size:11px;font-weight:600;color:var(--text-3);margin-bottom:8px;">${t('thickness')} (mm):</div>
            <div style="display:flex;gap:6px;flex-wrap:wrap;" id="detail-thick-wrap-${id}">${thickBtns}</div>
          </div>
          <div style="font-size:12px;color:var(--text-2);margin-bottom:14px;" id="detail-price-${id}">
            ${t('price')}: <b style="color:var(--red);font-family:var(--f-display);font-size:17px;letter-spacing:.3px;">${price.toLocaleString()} ${t('currency')}</b>
          </div>
...
function selectDetailThick(id, thick) {
  ...
  if (priceEl) priceEl.innerHTML = `${t('price')}: <b style="color:var(--red);font-family:var(--f-display);font-size:17px;letter-spacing:.3px;">${price.toLocaleString()} ${t('currency')}</b>`;
}
