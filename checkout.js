...
// ─── STEP 0: CART ─────────────────────────────────────────────
// Live update cart item total and grand total while typing
function updateCartItemLive(key, input) {
  ...
  if (itemRow) {
    const totalEl = itemRow.querySelector('.cart-total');
    if (totalEl) totalEl.textContent = (item.price * qty).toLocaleString();
  }
  // Update grand total
  const items = Cart.items();
  const grandTotal = items.reduce((s, i) => {
    const q = i.key === key ? qty : i.qty;
    return s + i.price * q;
  }, 0);
  const sumEl = document.querySelector('.cart-sum-val');
  if (sumEl) sumEl.textContent = grandTotal.toLocaleString() + " so'm";
  // Save to store
  Cart.setQty(key, qty);
}
...
function renderCart() {
      ...
      <div class="cart-info">
        ...
        <div class="qty-ctrl" style="width:90px;margin-top:5px">
          ...
          <input class="qty-val" type="number" min="1" value="${item.qty}"
            style="border:none;background:transparent;width:40px;text-align:center;font-family:inherit;font-size:inherit;font-weight:inherit;color:inherit;outline:none;-moz-appearance:textfield;cursor:text"
            onfocus="this.select()"
            oninput="this.value=this.value.replace(/[^0-9]/g,''); updateCartItemLive('${item.key}', this)"
            onblur="if(!this.value||this.value<1)this.value=1; Cart.setQty('${item.key}', this.value); renderCart()"/>
          <button class="qty-btn" onclick="Cart.updateQty('${item.key}',1);renderCart()">+</button>
        </div>
      </div>
  ...
  content.innerHTML = `
    ...
    <div class="cart-sum-row">
      ...
      <span class="cart-sum-val">${Cart.total().toLocaleString('ru-RU')} so'm</span>
    </div>
